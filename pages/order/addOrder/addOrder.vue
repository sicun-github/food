<template>
	<view class="container">
			...
			<view class="good">
				...
				<view class="List">
					...
					<view class="right">
						...
						<image
							src="../../../static/common/black_arrow_right.png"
							mode=""
						></image>
					</view>
				</view>
		...
		<view class="pay">
			...
			<view class="right">
				<image
					src="../../../static/purcase/weixin-pay.png"
					class="icon"
					mode=""
				></image>
		...
		<view class="settle">
			<view class="left">
				<text
					>合计：<text style="font-size: 34rpx; font-weight: bold">{{
						totalPrice
					}}</text></text
				>
			</view>
...
export default {
	...
	computed: {
		...mapState({
			shop_id: (state) => state.order.shop_id,
			takein: (state) => state.order.takein,
	...
	methods: {
		async getdefaultInfo() {
			...
			if (address.length > 0) {
				let defaultIndex = address.findIndex((item) => {
