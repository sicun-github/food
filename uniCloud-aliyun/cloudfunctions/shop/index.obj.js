...
module.exports = {
	...
	async getShopComment(id) {
		//获取店铺评价
		let dbJql = uniCloud.databaseForJQL();
		let shop_comment = await dbJql
			.collection('shop_comment')
			.where({
				comment_forShop: id,
			})
			.getTemp();
		let res = await dbJql
			.collection(shop_comment, 'user')
			.get()
			.catch((err) => {
				return {
					code: 500,
